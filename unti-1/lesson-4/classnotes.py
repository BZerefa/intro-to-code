...
# img = Image.open( sys.argv[1] )
# one_pixel = img.getpixel( (0,0) ) # made variable called one-pixel and saying for the image to get the pixels from top left corner in ( )

# print(one_pixel)

# img.putpixel( (10,10), (255,0,0) ) # put that pixel in coordinate 10,10 with an rgb value 
# img.save("new.jpg") # saving with new file name, can save as a different kind of file like png
# # when we printed, it shows the rgb value of that coordinate
...
# img = Image.open( sys.argv[1] )
# img_hsv = img.convert(mode="HSV") # converting it to different mode (HSV)
# img_hsv_data = img_hsv.getdata() # get data of HSV vlaue

# new_img_data = [] # make a new empty list out of a larger list
# for p in img_hsv_data: # running a loop
#     print(p)
#     if p[2] < 55: # if second value (saturation) is less than 55
#         new_img_data.append( (255,255,255) ) # append (add) these values to empty list
#     else: # let the regular pixels stay (rebuilding picture basiclly)
#         new_img_data.append(p)


# img_hsv.putdata(new_img_data) # putting data into image
# img_rgb = img_hsv.convert("RGB") # converting back to RGB
# img_rgb.save("filtered.jpg") # saving as a new file name (make sure your image is small)
...
# data = [] # need to understand what this code is doing
# for i in range(100):
#     pixel = (i, 0, 0)
#     data.append( pixel )
